package org.flixel
{
	...
	public class FlxObject extends FlxBasic
	{
		...
		/**
		 * If the object is invulnerable, then it doesn't take damage. Pretty simple.
		 */
		public var isInvulnerable:Boolean = false;
		/**
		 * For storing the damage this object deals.
		 */
		 ...
		 * 
		 * @param	Wound		How much health to take away (use a negative number to give a health bonus).
		 */
		public function hurt(Wound:Number):void
		{
			if (!isInvulnerable)
				health = health - Wound;
			if(health <= 0)
		...
		/**
		 * Grants the object experience due to events and calls a levelup if the object reaches the experience needed.
		 * 
		 * @param	Reward     Amount of experience rewarded for the event.
		 */
		public var currentExp:int;
		public var totalExp:int;
		public var rewardExp:int= 20;
		public var levelExp:int;
		public var levelBonus:int;
		public var currentLevel:int=0;
		public var levelExpArray:Array = [100,200,300,400,500,600,700,800,900,1000];
		public var maxLevel:int = 10;
		
		public function grantExp(Reward:Number):void
		{
			currentExp = currentExp + Reward;
			totalExp = totalExp + Reward;
			while (currentExp >= levelExp && currentLevel != maxLevel)
			{	
				currentExp = currentExp - levelExp;
				currentLevel += 1;
				levelExp = levelExpArray[currentLevel-1]
				//levelBonus += 1;
				//levelUp(levelBonus);
			}
		}
