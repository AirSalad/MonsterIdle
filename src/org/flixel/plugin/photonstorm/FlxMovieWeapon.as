...
package org.flixel.plugin.photonstorm 
{
	...
	import org.flixel.plugin.photonstorm.BaseTypes.BulletMovie;
	...
	public class FlxMovieWeapon //extends FlxObject
	{
		/**
		...
		public var currentMovieBullet:BulletMovie;
		...
		private var magCurrentCount:uint;
		private var magMaxSize:uint;
		private var magReloadDelay:uint = 0;
		private var magReloadTime:uint = 0;
		
		private var anglePerShot:uint;
		
		private var bulletType:String;
		
		public var damage:uint = 0;
		//Updated X & Y for collision triggers
		...
		//private static const FIRE_MULTIPLE_AT_MOUSE:uint = 6;
		 ...
		 */
		public function FlxMovieWeapon(name:String, parentRef:* = null, xVariable:String = "x", yVariable:String = "y")
		{
			this.name = name;
			
			bounds = new FlxRect(0, 0, 3600, 1760);// FlxG.width, FlxG.height);
		...
		public function makeMovieBullet(quantity:uint, movie:MovieClip, offsetX:int = 0, offsetY:int = 0, movieWidth:int = 1, movieHeight:int = 1, autoRotate:Boolean = false, rotations:uint = 16, frame:int = -1, antiAliasing:Boolean = false, autoBuffer:Boolean = false):void
		{
			//bulletType == "Movie";
			group = new FlxGroup(quantity);
			...
			for (var b:uint = 0; b < quantity; b++)
			{
				var tempBullet:BulletMovie = new BulletMovie(this, b);
				...
				{*/
					tempBullet.loadMovieClip(movie, movieWidth, movieHeight);
				//}
		...
		private function runFire(method:uint, x:int = 0, y:int = 0, target:FlxSprite = null, angle:int = 0):Boolean
		{
			...
			if (magMaxSize > 0 && (getTimer() < magReloadTime))
			{
				return false;
			}
			
			else if (magMaxSize > 0 && (getTimer() >= magReloadTime))
			{
				if (magCurrentCount > 0)
				{
					magCurrentCount--;
					trace("Magazine remaining: "+ magCurrentCount + "/" + magMaxSize);
				}
				else if (currentMagazine == 0)
				{
					magCurrentCount = magMaxSize;
					magReloadTime = getTimer() + magReloadDelay;
					trace("Magazine Reloaded");
					return false;
				}
			}
			...
			/*if (bulletsFired > 0)
			{
				for (var i:int = 0; i < bulletsFired; i++)
				{
					currentBullet = getFreeBullet();
					bulletArray[i] = currentBullet;
					trace(i + " Bullets loaded");
				}
			}
			else
			{*/
			/*if (bulletType == "Movie")
			{
				*/currentMovieBullet = getFreeMovieBullet();
				/*currentMovieBullet.gotoAndPlay("Basic");
			} else 
			{
				currentBullet = getFreeBullet();
			)
			...
			/*if (bulletType == "Movie")
			{*/
				currentMovieBullet.velocity.x = 0;
				currentMovieBullet.velocity.y = 0;
			/*}
			else
			{*/
			...
			/*if (bulletsFired > 0)
			{
				...
				{
					//currentBullet = getFreeBullet();
					bulletArray[i].velocity.x = 0;
					bulletArray[i].velocity.y = 0;
				}
			}*/
			...
			if (method == FIRE)
			{
				/*if (bulletType == "Movie")
				{*/
					currentMovieBullet.fire(launchX, launchY, velocity.x, velocity.y);
				/*}
				else
				{*/
			...
			else if (method == FIRE_AT_MOUSE)
			{
				/*if (bulletType == "Movie")
				{*/
					currentMovieBullet.fireAtMouse(launchX, launchY, bulletSpeed);
				/*}
				else
				{*/
			...
			/*else if (method == FIRE_MULTIPLE_AT_MOUSE)
			{
				if (bulletsFired > 1)
				{
					for (i = 0; i < bulletsFired; i++)
					{
						bulletArray[i].fireFromAngle(launchX, launchY, FlxVelocity.angleBetweenMouse(Registry.playerCenter, true), bulletSpeed);
						trace("Bullet " + i + " fired");
					}
				}
			...
			else if (method == FIRE_AT_POSITION)
			{
				/*if (bulletType == "Movie")
				{*/
					currentMovieBullet.fireAtPosition(launchX, launchY, x, y, bulletSpeed);
				/*}
				else
				{*/
			...
			else if (method == FIRE_AT_TARGET)
			{
				/*if (bulletType == "Movie")
				{*/
					currentMovieBullet.fireAtTarget(launchX, launchY, target, bulletSpeed);
				/*}
				else
				{*/
			...
			else if (method == FIRE_FROM_ANGLE)
			{
				/*if (bulletType == "Movie")
				{*/
					currentMovieBullet.fireFromAngle(launchX, launchY, angle, bulletSpeed);
				/*}
				else
				{*/
			...
			else if (method == FIRE_FROM_PARENT_ANGLE)
			{
				/*if (bulletType == "Movie")
				{*/
					currentMovieBullet.fireFromAngle(launchX, launchY, parent.angle, bulletSpeed);
				/*}
				else
				{*/
		...
		//public function fireMultipleAtMouse(bulletCount:int):Boolean
		//{
		//	return runFire(FIRE_MULTIPLE_AT_MOUSE,0,0,null,0,bulletCount);
		//}
		...
		public function setBulletType(type:String):void
		{
			bulletType = type;
		}
		/**
		 ...
		 */
		public function setBulletRandomFactor(randomAngle:uint = 0, randomSpeed:uint = 0, randomLifeSpan:uint = 0 ,randomPosition:FlxPoint = null):void
		{
		...
		public function setBulletMagazine(magSize:uint, magReloadSpeed:uint):void
		{
			magMaxSize = magSize;
			magReloadDelay = magReloadSpeed;
		}
		
		public function setBulletDamage(bulletDamage:int):void
		{
			//damage = _bulletDamage;
			group.setAll("damage", bulletDamage);
		}
		/**
		...
		private function getFreeMovieBullet():BulletMovie
		{
			var result:BulletMovie = null;
			...
			for each (var bullet:BulletMovie in group.members)
			{
				if (bullet.exists == false)
				{
					result = bullet;
		...
		public function fireSpread(angle:int, angleRange:int, amount:int):void
		{
			anglePerShot = angleRange / amount;
			if (getTimer() > magReloadTime)
			{
				for (var i:int = 0; i < amount; i++)
				{
					trace("Bullet fired at " + (anglePerShot* i));
					runFire(FIRE_FROM_ANGLE, 0, 0, null, (angle + (anglePerShot* i)));
				}
				for (var e:int = 1; e < amount; e++)
				{
					trace("Bullet fired at " + (anglePerShot* e));
					runFire(FIRE_FROM_ANGLE, 0, 0, null, (angle - (anglePerShot* e)));
				}
			}
		}
		
		public function fireMultiple(angle:int, amount:int):void
		{
			for (var i:int = 0; i < amount; i++)
			{
