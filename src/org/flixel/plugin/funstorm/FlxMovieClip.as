package org.flixel.plugin.funstorm
{
	...
	*/
	...
	public class FlxMovieClip extends FlxSprite 
	{
		...
		/**
		 * Stores the current playing animation
		 */
		public var _currentMCAnimation:String;
		...
		public function loadMovieClip(TheMovieClip:MovieClip, Width:int, Height:int, IsLooping:Boolean = false, AutoPlay:Boolean=false, CallbackOnComplete:Function = null):FlxMovieClip
		{
			...
			_currentMCAnimation = _mc.currentFrameLabel;
			return this;
		}
		...
		public function changeAnimation(newAnimation:String):void//(newAnimation:Number):void
		{
			//_currentMCAnimation = newAnimation;
			if (_mc.currentFrameLabel != newAnimation)
			{
				_mc.gotoAndPlay(newAnimation);
			}
			//trace(_mc.currentFrameLabel);
		}
		/**
		...
		override public function draw():void 
		{
			...
			if (_mc != null) { // Only draw if a movieclip has been loaded
				...
				while(i < l)
				{
					...
					camera.buffer.draw(_mcBitmapData,_matrix,null,blend,null,true); // where the magic happens: the bitmap data gets drawn to the camera
				}
			}
		}
